...
public class MenuScreen implements Screen {
    ...
    public MenuScreen(MasterSpaceShooter game, ResourceManager rm){
        ...
        this.game.rm.skin.getFont("font").getData().setScale(0.40f);
        this.game.rm.skin.getFont("font-title").getData().setScale(0.40f);

        this.stage.setViewport(this.game.viewport);
        this.root.setBounds(0, 0, MasterSpaceShooter.WORLD_WIDTH, MasterSpaceShooter.WORLD_HEIGHT);

        this.title[0] = new Label("MASTER", this.game.rm.skin, "title");
        this.title[1] = new Label("SPACE", this.game.rm.skin, "title");
        this.title[2] = new Label("SHOOTER", this.game.rm.skin, "title");

        this.play = new TextButton("Play", this.game.rm.skin, "orange-small-toggle");
        this.settings = new TextButton("Settings", this.game.rm.skin, "orange-small-toggle");
        this.exit = new TextButton("Exit", this.game.rm.skin, "orange-small-toggle");

        this.email = new TextField("email", this.game.rm.skin, "login");
        this.password = new TextField("password", this.game.rm.skin, "password");
        this.connectButton = new TextButton("Connect", this.game.rm.skin, "orange-small-toggle");
    ...
    private Label createLabel(final String text, final Color color) {
        ...
        style.font = this.game.rm.skin.getFont("font");
    ...
    public void setConnectMenu() {
        this.root.clear();
        this.root.add(this.title[0]).width(76).padLeft(34).row();
        this.root.add(this.title[1]).width(76).padTop(-5).padLeft(39).row();
        this.root.add(this.title[2]).width(76).padTop(-5).padLeft(27).row();
    ...
    public void setPlayMenu(){
        this.root.clear();
        this.root.add(this.title[0]).width(76).padTop(10).padLeft(34).row();
        this.root.add(this.title[1]).width(76).padTop(-5).padLeft(39).row();
        this.root.add(this.title[2]).width(76).padTop(-5).padLeft(27).row();
